...
export interface AiResult { action: AiAction, add: AiSuggestion[], cut: AiSuggestion[], note: string }
  ...
  async function run(action: AiAction, ctx: AiContext) {
    ...
    try {
      result.value = await $fetch<AiResult>('/api/ai/suggest', {
        method: 'POST',
        body: { action, commander: ctx.commander, identity: ctx.identity, cards: ctx.cards },
      })
    }
    catch (e: any) {
