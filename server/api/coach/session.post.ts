...
export default defineEventHandler(async (event): Promise<{ sessionId: string, continuationToken: string }> => {
  ...
  if (!message)
    throw createError({ statusCode: 400, statusMessage: locale(body) === 'en' ? 'A message is required' : 'Message requis' })
  // Cap the input so a giant deck/paste can't blow up the prompt or memory.
  if (message.length > 12_000)
    throw createError({ statusCode: 413, statusMessage: locale(body) === 'en' ? 'Message too long' : 'Message trop long' })
