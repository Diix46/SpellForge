...
export default defineEventHandler(async (event) => {
  ...
  const items = JSON.parse(record.items) as { printingId: string, finish: Finish, condition: Condition, quantity: number }[]
  const t = schema.collectionItems
  const mine = and(eq(t.userId, user.id), eq(t.game, record.game))
  const statements: BatchItem<'sqlite'>[] = [
    ...items.map(i => db.update(t).set({ quantity: sql`${t.quantity} - ${i.quantity}`, updatedAt: new Date() }).where(and(mine, eq(t.printingId, i.printingId), eq(t.finish, i.finish), eq(t.condition, i.condition)))),
